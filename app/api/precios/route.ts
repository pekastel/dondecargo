...
const createPriceSchema = z.object({
  ...
  fuente: z.enum(['oficial', 'usuario']).default('usuario'),
...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log('🔍 Starting create price request')
    ...
    const newPrice = await db
      ...
      .values({
        ...validatedData,
        fechaVigencia: validatedData.fechaVigencia 
          ? new Date(validatedData.fechaVigencia)
          : new Date(),
        esValidado: validatedData.fuente === 'oficial',
        fechaReporte: new Date(),
      })
      .returning()

    return NextResponse.json(newPrice[0], { status: 201 })
