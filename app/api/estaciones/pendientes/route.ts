...
export async function GET(request: NextRequest) {
  ...
  try {
    safeLog('🔍 Starting pending stations API request')
    ...
    if (userRole !== 'admin') {
      return NextResponse.json(
        { error: 'No autorizado. Solo los administradores pueden ver estaciones pendientes.' },
        { status: 403 }
      )
    }
    ...
    // Obtener estaciones pendientes con datos adicionales y usuario creador
    const pendingStations = await db
      .select({
        id: estaciones.id,
      ...
      .where(eq(estaciones.estado, 'pendiente'))
      .orderBy(estaciones.fechaCreacion)
    
    safeLog(`📊 Found ${pendingStations.length} pending stations`)
    
    // Formatear respuesta
    const formattedStations = pendingStations.map(station => ({
      id: station.id,
    ...
    if (error instanceof Error && error.message.includes('DATABASE_URL')) {
      return handleDatabaseError('pending stations API', error);
    }

    // Generic error handling
    return createErrorResponse(
      'pending stations API',
      error,
      500,
      'Error al obtener estaciones pendientes'
    );
  }
}
